...
async function loadMcpServers() {
  ...
  try {
    ...
    list.innerHTML = servers.map(s => {
      ...
      const statusColor = needsAnyAuth ? 'var(--warn)' : s.status === 'connected' ? 'var(--fg)' : s.status === 'error' ? 'var(--red)' : 'color-mix(in srgb, var(--fg) 50%, transparent)';
